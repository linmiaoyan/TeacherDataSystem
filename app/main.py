...
# 简单的session管理（生产环境建议使用更安全的方案）
admin_sessions = set()
teacher_sessions = {}  # {token: teacher_id}

def verify_admin_session(session_token: str = None) -> bool:
    """验证管理员session"""
...
def remove_admin_session(token: str):
    """移除管理员session"""
    admin_sessions.discard(token)

def verify_teacher_session(session_token: str = None) -> int:
    """验证教师session，返回teacher_id，如果无效返回None"""
    if not session_token:
        return None
...
def remove_teacher_session(token: str):
    """移除教师session"""
