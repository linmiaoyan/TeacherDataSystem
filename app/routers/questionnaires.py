...
@router.get("/", response_model=List[QuestionnaireResponseFull])
...
@router.get("/{questionnaire_id}", response_model=QuestionnaireResponseFull)
...
@router.post("/", response_model=QuestionnaireResponseFull)
def create_questionnaire(questionnaire: QuestionnaireCreate, db: Session = Depends(get_db)):
    """创建问卷"""
    # 转换字段格式
    fields_data = [field.dict() for field in questionnaire.fields]
...
def submit_response(response: QuestionnaireResponseCreate, db: Session = Depends(get_db)):
    """提交问卷回答"""
    # 检查问卷是否存在
...
@router.get("/responses/{response_id}", response_model=QuestionnaireResponseResponse)
...
@router.put("/responses/{response_id}", response_model=QuestionnaireResponseResponse)
...
@router.put("/{questionnaire_id}/close")
...
@router.get("/{questionnaire_id}/share-link")
