...
@router.post("/{task_id}/complete-export")
...
def query_teacher_tasks(query: TeacherQuery, db: Session = Depends(get_db)):
    ...
    # 查找包含该教师的任务
    # 注意：teacher_ids是JSON字段，需要特殊处理
    # 由于SQLAlchemy的JSON字段contains方法可能不准确，我们查询所有任务然后过滤
...
def get_my_tasks(
    ...
    """获取当前登录教师的任务列表（需要登录）"""
    from app.main import verify_teacher_session
    from app.models import Teacher
...
def download_teacher_file(
    ...
    # 如果提供了token，验证token并确保teacher_id匹配
    token = request.headers.get("X-Teacher-Token") or request.cookies.get("teacher_token")
    if token:
        verified_teacher_id = verify_teacher_session(token)
        if verified_teacher_id and verified_teacher_id != teacher_id:
            raise HTTPException(status_code=403, detail="您无权访问此文件")
        if verified_teacher_id:
            teacher_id = verified_teacher_id  # 使用token验证的teacher_id
